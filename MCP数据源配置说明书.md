...
> 适用范围：QuantLab 全部项目，重点是 Project\_16\_LightGBM股票大师 的实盘链路
>
> 数据依据：2026-08-26 全量连通性实测校准（38 个股市 MCP server 逐个调用验证，非凭记忆）+ **2026-08-30 full-link 内置 TDX 端点复核（判定为下线）**
>
...
| ------------------------------- | --------------------------------------- | ---------------------------- | --------------------------- | ------------------------------------------------------------------------------------------- |
| `mcp_tdx`                       | TRAE 全局挂载的自建 TDX 服务                     | **✅ 可用·一级主力**                | 约 40ms 实测                   | 批量行情首选，免 Key；代码格式仅接 `sh600519`/`sz300684`（6位+小写前缀），**不接** **`600519.SH`** **后缀**（后端 reject） |
...
| `full-link` 内置 TDX              | full-link-stock-analysis 插件自带           | **❌ 端点下线·六级失效**              | 不可达（2026-08-30 复核）          | 原定位"单股字段最全的冗余位"，现 TCP/端口全不通；详见链路说明"四点五"                                                     |
...
| `mcp_wudao`（悟道）                 | 远程 HTTP 型 MCP，Bearer 鉴权（直连客户端 scripts/wudao_client.py，T-20260903-001） | 可用·特色源（2026-09-03 改直连） | 秒级（MCP JSON-RPC）            | 新闻热榜/涨停梯队/龙虎榜/题材热度/盘中主力/竞价等信息面；定时任务一律走直连客户端，勿依赖插件层（9-02/9-03 连挂根因已根治） |
| `sina-finance`（新浪智研）            | 远程 MCP（streamable-http，X-Auth-Token，收费） | 可用·F3/F5 第二顺位（2026-08-31 接入） | **约 60-80ms 实测**            | 个股新闻（F3 催化）/实时行情/板块涨停/估值/北向/融资融券/财务；**无 A 股主力资金流接口（F2 不可用）**，收费源预期稳定                        |
| `tushare`（Tushare moneyflow）         | API（token，200元/2000积分档包年·调用不消耗积分） | **P16 F2 模型特征权威主源**（2026-09-03，T-20260903-004） | 盘后 T-1（19:30 增量刷）         | 主力资金流四档（`buy_lg`/`buy_elg`/`net_mf_amount` 等 18 列）**与训练口径零 skew**；单位=万元（**禁止 ÷1e4**，2026-09-03 实测修正）；`scripts/refresh_tushare_moneyflow.py` + Schedule「Tushare刷新」c63cff58 19:30；评分卡实时 F2（9:45 当日）仍用东财/悟道 |
| 万得 Wind（CLI）                    | wind-mcp-skill 官方 CLI                   | 可用·备用                        | 约 952ms（含分钟线大返回）            | 港美股/债券/宏观/公告专业深研，单标的耗积分                                                                     |
| TuShare                         | 远程 MCP（220+ 接口）                         | 可用·备用                        | 视接口（免费版限频：trade\_cal 1次/小时） | 沪深股票/指数/财务/资金流/龙虎榜，需 Token                                                                  |
...
### 统一优先级体系（2026-09-03 实测定版 v8，按职责分组）

> v8 更新（2026-09-03，T-20260903-002）：①**F3/F5/基础行情/大盘指数链均加入悟道直连备选**（F3 用 `official_announcements`/`research_reports`/`cls_news`；F5 用 `theme_intraday_capital`/`sector_analysis`；基础行情用 `stock_rank`/`valuation_snapshot`；指数用 `index_market`）；②**新增熔断路由 `scripts/data_source_router.py`**——连续失败 3 次熔断 300s，取数前 `check`（OPEN 跳过）/ 调用后 `record ok|fail`，快速切可用源、杜绝死源耗超时。四任务取数规则已固化。

> v7 更新（2026-09-03，T-20260903-001）：悟道改**直连客户端** `scripts/wudao_client.py`（绕开 TRAE MCP 插件层——定时任务环境插件不可达是 9-02/9-03 连挂根因；实测 `/api/mcp` JSON-RPC POST + Bearer 可用、63 工具全通、key 盘中可用；`/api/openclaw` REST 404 不存在已修正）。F2/F3/F5 链中悟道调用一律 `python scripts/wudao_client.py capital_flow '{"flowType":"stock","stockCodes":[...]}'`。

> v6 更新（2026-08-31）：①F2 资金源修复（iFind 前一日口径虚高实锤 T-20260831-003，改用东财当日）；②F3 催化 / F5 板块加 `sina-finance`（新浪智研，收费源）为**第二顺位**（mcp_tdx 之后，实测 ~60-80ms，T-20260831-004）。

> v5 优化背景（2026-08-31 实测台账）：`mcp_tdx` 一级主力在 6 个交易日里 3 天异常/掉线（08-24 字段异常、08-25 掉线、08-31 503），而腾讯 API 全勤零失败、iFind 在 TDX 缺席时能独立扛起扩展指标。原"mcp\_tdx 打头"的全链路降级导致 9:45 频繁来回切源。v5 改为**按指标职责分组**：基础行情锚定腾讯（最稳），扩展指标 mcp\_tdx 优先 + iFind 固定兜底，TDX 系不可达绝不阻塞。

取数分三组，组内前一级失败或字段缺失才降级到下一级：

**① 基础行情（价/量/量比/PE/换手/涨跌停价）**

1. 腾讯 API（`qt.gtimg.cn`）—— **最稳锚定源**（实测全勤零失败），免费毫秒级
2. `mcp_tdx` —— 增强覆盖位（能取到则覆盖为更实时值/更全字段；不可达不阻塞，腾讯数据已在手）

**② 扩展指标（主力资金 F2 / 催化 F3 / 板块 F5）**

0. `tushare`（Tushare moneyflow）—— **F2 模型特征权威主源**（四档与训练口径零 skew，单位=万元**禁止 ÷1e4**；19:30 刷 T-1，`scripts/refresh_tushare_moneyflow.py` + Schedule「Tushare刷新」c63cff58；`deploy_predict_g2.py` 已自适应：快照新鲜→主源、滞后→回退新浪兜底；T-20260903-004）。⚠️ **与评分卡实时 F2 区分**：评分卡 F2（9:45 当日实时）仍用东财/悟道当日口径，Tushare T-1 日频只做模型特征层（`mf_main_net`/`mf_elg_net`/`mf_main_ratio` 等 g2 43 特征）
1. `mcp_tdx` —— 字段最全（资金流/板块涨幅/催化，当日口径）
2. 悟道 `capital_flow`（flowType=stock）—— **F2 主力资金固定第二顺位**（东财当日四档，2026-08-31 实测当日真实；原 iFind 前一日口径且方向存疑，已实锤 300456 虚高 TOP1，T-20260831-003）。**调用方式：`python scripts/wudao_client.py capital_flow '{"flowType":"stock","stockCodes":[...]}'` 直连 `/api/mcp`**（2026-09-03 改，勿依赖 mcp_wudao 插件，T-20260903-001）
3. 东方财富 mx —— `mx_ashare_finance_data` 当日资金流/研报（可替代东财 curl）
4. 东财 curl —— 备选（避免高频触发限频）
5. iFind —— **仅兜底**：主力资金为**前一日口径**，若使用必须写 `main_net_inflow_date`（review\_full 时效校验：非当日数据 F2 按缺失 5 分处理，绝不用旧数据打高分）；F3 催化/新闻/公告可用
6. `sina-finance`（新浪智研）—— **F3 催化 / F5 板块第二顺位**（mcp\_tdx 之后：个股新闻 `stockNewsSearch` / 强势板块 `cnMarketStrongSectors` / 虚拟板块 `cnVirtualSectorRanking` / 估值 `cnStockValuationDetail`；实测 \~60-80ms 收费源稳定；**无 A 股主力资金流，不参与 F2**，T-20260831-004）

**③ F3 催化完整链（v8，2026-09-03 加悟道备选）**

`mcp_tdx` → `sina-finance`（stockNewsSearch 个股新闻）→ **悟道直连**（`official_announcements` 公告 / `research_reports` 研报 / `cls_news` 财联社快讯）→ `iFind` → `eastmoney_mx` → `eastmoney_curl`

**④ F5 板块完整链（v8，2026-09-03 加悟道备选）**

`mcp_tdx` → `sina-finance`（cnMarketStrongSectors/cnVirtualSectorRanking）→ **悟道直连**（`theme_intraday_capital` 题材资金 / `sector_analysis` 板块分析）→ `iFind` → `eastmoney_mx` → `eastmoney_curl`

**⑤ 基础行情完整链（v8，2026-09-03 加悟道补充）**

`腾讯 API`（最稳锚定）→ `mcp_tdx`（覆盖增强）→ **悟道直连**（`stock_rank` / `valuation_snapshot` 补充）→ miniQMT 兜底

**⑥ 大盘风控指数链（v8）**：腾讯 API 沪深300 → `tdx_get_index_quote("000300")` → **悟道直连 `index_market`** → iFind `index_highfreq_quotes`

**⑦ 熔断路由（2026-09-03，T-20260903-002，`scripts/data_source_router.py`）**

各源不稳定（TDX 6日3挂/悟道插件连挂/iFind 口径存疑），引入熔断器快速短路故障源、自动切可用源：**取数前 `python scripts/data_source_router.py check <源>`（OPEN 跳过 / PROBE·OK 才调用）→ 调用后 `record <源> ok|fail [耗时ms]`**。连续失败 ≥3 次熔断 300s，熔断期过半开探测，成功立即恢复。状态持久化 `data/cache/datasource_health.json`（多任务共用）。9:25/9:45/11:35/15:40 四任务取数规则已固化该机制。

**⑧ 冗余位（原则上不动）**
5\. full-link 内置 TDX —— **2026-08-30 复核端点下线（DNS 通 TCP 全拒），实际不可用**，跳过
6\. 独立 `mcp_plugin_TDX_tdx` —— 冗余位（已恢复可用，勿再标记禁用）；full-link 失效后顺位自动吸收为新的第 5 级
7\. miniQMT —— 兜底，注意红线（见第八章）
8\. 其他来源 —— 原则上不使用

悟道不参与行情优先级排序，作为信息面/情绪面的并行特色补充，与行情链路独立。**调用一律走 `scripts/wudao_client.py` 直连**（勿依赖 mcp_wudao 插件——定时任务环境插件不可达，T-20260903-001）。
...
### tdx\_get\_quote 批量实时行情

**代码格式是本工具最大的坑，必须用** **`sh`/`sz`** **小写前缀 + 六位数字，不接** **`.SH`/`.SZ`** **后缀**（2026-08-30 实测 600519.SH 被后端 reject，`sh600519` 才能通）：

```
正确：tdx_get_quote(codes="sh600519,sz300684")
错误：tdx_get_quote(codes="600519.SH,300684.SZ")
      → Error executing tool tdx_get_quote: invalid code: '600519.sh'
错误：tdx_get_quote(codes="600519,300684")
      → 旧版写法，2026-08-30 验证已不通过
```

交易所靠 `sh`/`sz` 前缀显式声明，不能省略也不能写在后缀位置；批量查询时所有代码必须在同一 `codes` 字符串中用半角逗号分隔，不接空格。
...
**call\_auction 两个字段是隐藏亮点**：9:25 集合竞价结束后即反映当日竞价结果，9:25 定时任务可直接从普通行情快照读取竞价成交额与竞价涨幅，无需额外接口。
...
唯一使命：补齐 mcp\_tdx 缺失的五类字段——**量比、PE(TTM)、PB、流通市值、涨跌停价**。
...
* 不覆盖集合竞价量/涨幅（call\_auction 保留 mcp\_tdx）、不覆盖题材热榜/情绪温度（保留悟道）
...
## 七、悟道 MCP（信息面特色源，2026-09-03 改直连客户端）

> **2026-09-03 变更（T-20260903-001）**：定时任务环境曾因 TRAE 插件层不可达而"连挂"（9-02/9-03）。已新增 `scripts/wudao_client.py` **urllib 零依赖直连 `/api/mcp`**（JSON-RPC POST + Bearer key），任务指令统一用 `python scripts/wudao_client.py <tool> '<json_args>'` 调用；实测 63 工具全通、key 盘中可用（13:12/15:05 均调通，排除 FREE_TIER_MARKET_OPEN_RESTRICTED 盘中受限）。项目 `.mcp.json` 已加 `wudao`（streamable-http + Bearer，gitignore 保护）。
...
### 直连/降级通道（2026-09-03 实测修正）

⚠️ **`/api/openclaw` REST 路径实测 404 不存在**（早期文档推断值，勿再使用）。悟道真实通道只有 **MCP 端点 `/api/mcp`（JSON-RPC over POST + Bearer）**；MCP 插件不可用时，直接 `python scripts/wudao_client.py <tool> '<json_args>'` 走同一端点（零依赖，仅 urllib），即推荐的标准调用方式（T-20260903-001）。
...
* 直连模式：Project\_16 内 `qmt_trader.py`、`qmt_monitor.py` 等，交易动作一律走本地 QMT 客户端
...
以下数据源已实测连通，作为 iFind/mcp\_tdx 之外的补充入口。其中东方财富 mx 已提升为**四级金融主力**参与行情优先级排序（见第一章），其余按备用用途使用：
...
| 任务       | 时间        | 状态                                                                                                                                                                                                                                                                |
| -------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 集合竞价增强预判 | 交易日 09:25 | 已生效（v8）：腾讯竞价/量比兜底 + mcp\_tdx 覆盖增强（`tdx_get_call_auction` 直读竞价序列）+ 扩展指标 mcp\_tdx→**悟道直连 capital\_flow（scripts/wudao_client.py，F2 东财当日）**→东财mx→东财curl（iFind 兜底）；F3 催化 mcp\_tdx→sina-finance→**悟道直连（公告/研报/财联社）**→iFind→东财mx→东财curl；F5 板块 mcp\_tdx→sina-finance→**悟道直连（题材资金/板块分析）**→iFind→东财mx→东财curl；**全链熔断路由 scripts/data_source_router.py（T-20260903-002）** + 悟道直连四工具判情绪                                                                           |
| 开盘实时复核   | 交易日 09:45 | 已生效（v8）：腾讯基础行情兜底 + mcp\_tdx 覆盖增强 + 悟道直连 stock\_rank/valuation\_snapshot 补充；**F2 主力资金当日口径** mcp\_tdx→悟道直连 capital\_flow（scripts/wudao_client.py）→东财mx→东财curl（iFind 兜底写日期戳）；F3 催化 mcp\_tdx→sina-finance→**悟道直连（公告/研报/财联社）**→iFind→东财mx→东财curl；F5 板块 mcp\_tdx→sina-finance→**悟道直连（题材资金/板块分析）**→iFind→东财mx→东财curl；大盘风控指数源腾讯沪深300→`tdx_get_index_quote("000300")`→**悟道直连 index\_market**→iFind；**全链熔断路由** |
| 午休持仓报告   | 交易日 11:35 | 已生效（v8）：腾讯基础行情 + mcp\_tdx 增强 + 悟道直连补充；扩展指标 mcp\_tdx→悟道直连 capital\_flow→东财mx→东财curl；F3 催化 mcp\_tdx→sina-finance→**悟道直连（公告/研报/财联社）**→iFind→东财mx→东财curl；F5 板块 mcp\_tdx→sina-finance→**悟道直连（题材资金/板块分析）**→iFind→东财mx→东财curl；**全链熔断路由** + 悟道直连午间热榜/公告                                                                                                                                                                                           |
| 盘后持仓复盘   | 交易日 15:40 | 已生效（v8）：腾讯基础行情 + mcp\_tdx 增强 + 悟道直连补充；扩展指标 mcp\_tdx→悟道直连 capital\_flow→东财mx→东财curl；F3 催化 mcp\_tdx→sina-finance→**悟道直连（公告/研报/财联社）**→iFind→东财mx→东财curl；F5 板块 mcp\_tdx→sina-finance→**悟道直连（题材资金/板块分析）**→iFind→东财mx→东财curl；**全链熔断路由** + 悟道直连盘后公告/龙虎榜/梯队复盘                                                                                                                                                                                     |
...
| `wudao`         | http  | `https://stock.quicktiny.cn/api/mcp` + `Authorization: Bearer ${WUDAO_API_KEY}`   | 悟道信息面；**Key 走环境变量** **`${WUDAO_API_KEY}`，不落盘明文**，运行环境需 `setx WUDAO_API_KEY <Key>`；**定时任务调用一律 `python scripts/wudao_client.py <tool> '<json_args>'` 直连（勿依赖插件层，T-20260903-001）** |
...
## 十二、策略数据覆盖矩阵（2026-08-30 核对）

策略数据需求分两层：**离线训练/回测层**（本地 E 盘存量库 + 增量目录）与**实时生产层**（MCP 降级链 + 信息面）。逐项核对结论：**无任何"需要但无源"的数据缺口，当前配置可完全覆盖**。

### 离线训练/回测层

训练/回测数据来自本地 `E:\astock` 买断库（Tushare 口径、Parquet 格式）+ `E:\astock\Updatedata` 增量目录，**不依赖任何 MCP 实时源**，离线侧零缺口：

| 需求                                  | 数据源                                                                                                 | 状态     |
| ----------------------------------- | --------------------------------------------------------------------------------------------------- | ------ |
| 历史日线 OHLCV                          | `E:\astock\daily\stock_daily.parquet`（2009 至今，5826+ 只，1.27GB）                                       | ✅ 本地仓  |
| 财务 8 表（指标 167 字段/预告/快报/三大表/分红/股本变动） | `E:\astock\finance\*.parquet`（2005Q1\~2026Q1）                                                       | ✅ 本地仓  |
...
| 专题数据（资金流/龙虎榜/北向/筹码/板块/竞价/研报/指数成分）   | `E:\astock\moneyflow\` `lhb\` `northbound\` `chip\` `board\` `auction\` `research\` `index_weight\` | ✅ 本地仓  |
| 行情增量（每周）                            | `E:\astock\Updatedata\<周>\`（含日线/1/5/15/30/60min/基础信息）                                               | ✅ 增量目录 |

### 行情增量机制（2026-08-28 接入）

用户约定：**每周行情增量数据**以"周区间"子目录放入 `E:\astock\Updatedata\`，研发/回测按需调用，不合并进主仓：

* **主仓保持只读**：`E:\astock\daily\stock_daily.parquet` 不修改；增量文件独立保留在 `Updatedata\<周>\` 内（全市场单文件格式）

* **统一读取入口**：`data_config.read_main_daily(columns=...)` 返回主仓 + 增量合并结果（增量优先、按 `(trade_date, ts_code)` 去重），返回 MultiIndex 与主仓格式一致

* **自动扫描**：`data_config.list_update_weeks()` 自动发现新周目录，无需改代码

* **回测引擎**：`AstockParquetReader`（`D:\QuantLab\data\astock_reader.py`）实例化时自动合并增量，回测代码零改动

* **已接入**：`build_features.py` 特征构建已改为调用 `read_main_daily()`

* **增量目录文件**：`stock_daily.parquet` / `stock_1min_data.parquet` / `stock_5min_data.parquet` / `stock_15min_data.parquet` / `stock_30min_data.parquet` / `stock_60min_data.parquet` / `stock_basic_data.parquet`

* **更新中转**：`update_staging\` 暂存待整合增量（当前 `merge_20260828\1min` 待整合）；`update_logs\` 记录更新日志；`backup_YYYYMMDD\` 更新前备份
...
| 需求                 | 主源                     | 备源                      | 状态     |
| ------------------ | ---------------------- | ----------------------- | ------ |
| 基础行情（现价/开高低/量额）    | mcp\_tdx               | 腾讯                      | ✅      |
| 量比/PE/PB/流通市值/涨跌停价 | 腾讯                     | full-link               | ✅      |
| 主力资金净流入(F2)       | 悟道直连 capital\_flow（scripts/wudao_client.py，东财当日四档） | 东财mx → 东财curl → iFind（仅兜底带日期戳） | ✅ 三重冗余 |
| 板块涨幅(F5)          | mcp\_tdx / sina-finance / **悟道直连（theme_intraday\_capital/sector\_analysis）** | 东财mx / iFind             | ✅      |
| 集合竞价量/涨幅           | mcp\_tdx call\_auction | tdx\_get\_auction\_0925 | ✅      |
| 沪深300大盘风控          | mcp\_tdx index / **悟道直连 index\_market** | 腾讯/full-link/iFind      | ✅ 四重通道 |
| 新闻催化(F3)           | sina-finance / **悟道直连（official\_announcements/research\_reports/cls\_news）** | 东财mx/iFind               | ✅      |
| 龙虎榜/涨停梯队/题材热度      | 悟道直连（scripts/wudao_client.py） | iFind                   | ✅      |
| 公告                 | 悟道直连（scripts/wudao_client.py） | iFind/东财mx              | ✅      |
| 全链熔断路由            | scripts/data_source_router.py（check/record，3 次失败熔断 300s） | —                        | ✅ v8   |
| 选股/财务补充            | iFind                  | 东财mx                    | ✅      |
...
* **离线层**：`E:\astock` 本地仓（训练/回测/因子构建）+ `E:\astock\Updatedata` 每周增量（按需合并，`read_main_daily()` 读取）

* **实时层**：`mcp_tdx → 腾讯 → iFind → 东财mx → 东财curl → full-link → 独立TDX → miniQMT` 八级降级链 + 悟道信息面（`scripts/wudao_client.py` 直连，T-20260903-001）+ 备用深研源（万得/盈米/TuShare/AKShare/tradingagents）

### 已知边界（非缺口，需留意）

1. **离线仓时效**：财务数据截至 2026Q1、数据仓整理于 2026-06-26；历史数据滞后由实时 MCP 补最新，不影响训练；行情增量已通过 `Updatedata` 周目录补充（截至 8-21）
2. **30min/60min 长历史仍为空**：`E:\astock\minute\` 下这两档主仓无数据（原始采购未提供）；`Updatedata` 周增量已含这两档，但仅覆盖增量周，如需 2009 至今长历史仍需另购
3. **TuShare 免费版限频**（trade\_cal 1 次/小时）：只适合离线补历史，不适合盘中采集
