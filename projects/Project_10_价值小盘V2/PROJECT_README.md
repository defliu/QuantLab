...
> **2026-08-17 研究备注（股息率过滤）**：V2a + TTM现金股息率>=1% 合格域过滤消融通过否决性判据（2024+超额+39.6% vs 基线+35.2%，2026超额+1.4%），且回撤从-29.7%降至-28.9%，但绝对年化从+16.2%下滑至+14.7%，候选池压缩43%。**归档为防御性微调，不部署**（优先级=清脏仓+模拟盘验证V2.3）。详见 `results/d_dividend_screen.txt` / `research/d_dividend_screen.py`。
