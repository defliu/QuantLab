...
def _load_st_set(C):
    """从 QMT 风险警示板拉取 ST 股票名单（一次性，避免逐股查名称）
    仅在需要调仓时调用。失败时返回空集合（安全降级：不过滤）。"""
    st_set = set()
    try:
    ...
    if not st_set:
        try:
            ...
            pass
    if not st_set:
        _log("[st] 警告: ST板块名单获取失败，ST过滤本次跳过（保守降级）")
    else:
        _log("[st] ST股筛除加载成功: %d 只" % len(st_set))
    return st_set
            ...
            if od["dir"] == "buy":
                ...
                if tr and tr["sell_vol"] > 0:
                    ...
                    if pos is None or acct_vol <= 0:
                        # 订单查询成功但账户确无货（含幽灵持仓 vol=0，当日卖出残留）：实际已卖出
                        _cash += od["amount"] * od["price"]
                        _holdings.pop(code, None)
                        _entry_prices.pop(code, None)
                        _entry_dates.pop(code, None)
