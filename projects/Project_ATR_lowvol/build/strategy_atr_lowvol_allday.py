...
def init(C):
    global _g_runner
    print("  [ATR诊断] init 被调用")
    _g_runner = StrategyRunner()
    _g_runner.init(C)
...
def handlebar(C):
    global _g_runner, _g_hb_count
    _g_hb_count += 1
    # 限频打印：QMT启动时历史K线回放会瞬间调用数千次，全量打印会刷爆日志
    if _g_hb_count <= 3 or _g_hb_count % 500 == 0:
        print("  [ATR诊断][v4] handlebar 第%d次, _g_runner=%s" % (_g_hb_count, str(_g_runner is not None)))
    if _g_runner is not None:
        try:
            _g_runner.handlebar(C)
        except KeyboardInterrupt:
            print("  [策略] 已手动停止，正在退出...")
            raise
