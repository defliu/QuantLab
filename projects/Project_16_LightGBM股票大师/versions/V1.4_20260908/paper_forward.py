...
# 位置过滤开关（T6，2026-09-08 落地）：--replay 回放与实盘 G2（full）同口径，
# 由环境变量 BT_POSFILTER 控制（默认 off 保持历史回放不变）。
POSFILTER_MODE = os.environ.get("BT_POSFILTER", "")
...
def main():
    ...
    for i, d in enumerate(dates):
        ...
        # 位置过滤（T6）：显式设置 BT_POSFILTER=lite/full 时启用，与实盘候选同口径
        if POSFILTER_MODE and len(cand) > 0:
            cand = PF.apply_rules(cand.reset_index(), target_date=d, mode=POSFILTER_MODE)
            if len(cand) == 0:
                continue
