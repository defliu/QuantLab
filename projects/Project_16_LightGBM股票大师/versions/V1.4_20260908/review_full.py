...
def main():
    ...
    # ---- 位置过滤（V1.4，T2）：V1.3 红线58 口径验证后仅用 lite（R2 高位剔除）----
    # 依据 2026-09-08 三步验证：58-lite 日均超额 +0.157% 有效；full（R1/R3）在 58 下过严
    # （+0.010%）会误伤，故 V1.3 不开 R1/R3。PF_DISABLE=1 一键回滚。
    _reasons = {}
    _n_before = len(out)
    out = PF.apply_rules(out, target_date=str(rdate),
                         mode=os.environ.get("PF_MODE_V13", "lite"), reason_out=_reasons)
    if _reasons:
        print(f"    !! 位置过滤(lite)剔除 {len(_reasons)}/{_n_before} 只: {_reasons}")
    if len(out) == 0:
        print("    !! 位置过滤后无候选 → 空仓（宁缺毋滥）")
        return
    print("[2/3] 完整版评分：")
