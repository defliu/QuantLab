# coding: utf-8
"""融合(ensemble) 每日 positions_cfg（成本锚）自动生成 —— 桥内止损/止盈/对账账本依赖。

数据源：
  1) 融合持仓 code 集合：data/rebalance_g2_ens/g2_ens_hold_dates.json（建仓日记录 = 融合纳管持仓）
  2) 成本/数量：state/positions_<date>.json（桥每日导出账户全量持仓，avg_price=含费成本）

只写融合自己的持仓（绝不把账户里他人策略/孤儿的票写进融合成本锚），account_id 戳校验。

用法：
  python gen_positions_cfg_ens.py                # 今天（date=今日）
  python gen_positions_cfg_ens.py --date 20260902
  python gen_positions_cfg_ens.py --dry-run      # 只打印不写
...
from qmt_bridge_client_ens import read_positions, _atomic_write_json, _positions_cfg_path, STATE_DIR, _read_json
...
def _load_hold_codes():
    """融合持仓 code 集合（g2_ens_hold_dates.json 的 key；文件缺失返回空集）。"""
    try:
...
def main_with_date(date, dry_run=False):
    """按指定 date 生成 positions_cfg（供 CLI 与 rebalance_ens 复用）。返回退出码。"""
    print("== 融合 positions_cfg 生成（%s）== account=%s" % (date, G.ACCOUNT_ID))

    hold_codes = _load_hold_codes()
    if not hold_codes:
        print("[SKIP] g2_ens_hold_dates.json 无融合持仓记录（空仓或未初始化），不写成本锚")
        return 0
    ...
    if not rows:
        print("[SKIP] 融合持仓在账户中都无有效仓位，不写成本锚")
        return 0
...
def main():
    ap = argparse.ArgumentParser(description="融合每日 positions_cfg 成本锚自动生成")
