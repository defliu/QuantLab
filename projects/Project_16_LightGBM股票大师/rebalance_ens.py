# coding: utf-8
"""融合(ensemble)（大QMT 文件桥）每日换仓执行 —— 与 G2 桥（g2_bridge）**完全隔离**。

流程（09:50 开盘后执行，先卖后买）：
  1) 读融合选股 data/selections/g2_ens/<date>_g2_top10.csv（deploy_predict_g2 --ensemble --ens-live 产出）
  2) 读融合账本持仓（只认融合自己的：cmd/positions_cfg_<date>.json 成本锚 + fills FIFO 推导；
     绝不读/纳管账户全量持仓里他人的票）
  3) 卖出：融合账本中不在目标 topN 的 → SELL（可卖量受 can_use 限制，T+1 锁定不卖）
  4) 买入：目标中持有不足的 → BUY 补到目标股数（等权对齐资金池 95%/N，整手）
     重叠规避：跳过 G2 账本（g2_hold_dates.json）已持有的票，防同账户双桥争同一持仓
  5) 写 orders 到桥 cmd/orders_<date>.json（dry-run 默认，--live 才真写）
  6) 输出 data/rebalance_g2_ens_<date>.md/.json

用法：
  python rebalance_ens.py                  # dry-run（默认）
  python rebalance_ens.py --date 20260902  # 指定日期
  python rebalance_ens.py --live           # 真写桥（慎用，先卖后买）
"""
import argparse
import csv
import datetime
...
from qmt_bridge_client_ens import (
...
def _gen_positions_cfg(date):
    """生成当日 positions_cfg 成本锚（桥内止损/对账账本依赖）。
    复用 gen_positions_cfg_ens.py 逻辑：融合持仓 code（g2_ens_hold_dates.json）∩ 账户持仓
...
def _latest_avail_candidate(date, top_n):
    """扫描 g2_ens/ 目录取最新数据日的候选；新鲜度须 == 增量库最新日，否则宁缺毋滥中止。

    候选文件以「数据日」命名（deploy_predict_g2 --ensemble --ens-live 输出，如 20260902_g2_top10.csv），
    而调用方传「运行日」（如 20260902）。运行日候选缺失属正常（候选天然滞后一天），
    自动回退最新数据日候选；但若其数据日落后于增量库最新日，说明 09:25 候选生成失败，
    绝不用过期候选交易。绝不回退 G2/V1.3 的选股文件（隔离红线）。
    返回 (csv_path, note)；无可用候选返回 (None, 原因)。
    """
    sel = G.ENS_SELECT_DIR
    if not os.path.isdir(sel):
        return None, "融合选股目录不存在: %s" % sel
    ...
    if not cands:
        return None, "融合选股 CSV 不存在: %s" % os.path.join(sel, "%s_g2_top%d.csv" % (date, top_n))
    cands.sort()
    d, path = cands[-1]
    latest_incr = _latest_incr_date()
    if latest_incr and d != latest_incr:
        return None, ("融合候选数据日 %s 落后于增量库最新日 %s（09:25 候选生成失败？），"
                      "宁缺毋滥中止，绝不用过期候选" % (d, latest_incr))
    return path, "运行日候选缺失，回退到最新数据日 %s 的候选" % d


def load_ens_selection(date, top_n):
    """读融合选股 CSV → 候选池（total>=REDLINE，按 total 降序；不截断，买入侧自行排除持仓取最高）。

    先按运行日 <date> 精确匹配；缺失则回退最新数据日候选（见 _latest_avail_candidate）。
    绝不回退 G2/V1.3 的选股文件（融合/G2 隔离红线）。
    """
    csv_path = os.path.join(G.ENS_SELECT_DIR, "%s_g2_top%d.csv" % (date, top_n))
    note = "融合候选池 %d 只（红线 %.0f）" % (top_n, G.REDLINE)
    ...
    if not picks:
        return [], "融合选股无 total>=%.0f 的票（宁缺毋滥）" % G.REDLINE
    picks.sort(key=lambda p: p["total"], reverse=True)
    return picks, "融合候选池 %d 只（红线 %.0f）" % (len(picks), G.REDLINE)


def load_ens_ledger(date):
    """融合账本持仓：positions_cfg（成本锚）为主 + fills FIFO 推导兜底。
    返回 {code: {"vol": int, "cost": float, "src": str}}。绝不读账户全量持仓。"""
    ledger = {}
    # ① positions_cfg（融合自写成本锚 = 纳管持仓）
    #    当日缺失 → 回退最近可用 positions_cfg（换仓时当日 positions_cfg 由盘前/15:40 生成；
    #    若缺失且 fills 未落盘 → 账本空 → 误判"无持仓"而错误买入 → 回退昨日账本）
...
def build_plan(date, capital):
    ...
    返回 (orders, plan_dict)。"""
    pool, note = load_ens_selection(date, G.SELECT_TOP)   # 候选池（Top10）
    ledger = load_ens_ledger(date)
    ...
    for code, ld in ledger.items():
        ...
        orders.append({
            ...
            "reason": "融合持有到期(满%d日)" % G.HOLD_DAYS, "strategy_order_id": "P16_%s_%04d" % (date, seq0 + len(orders) + 1),
        })
    ...
    if n_slots > 0 and pool:
        ...
        if g2_held:
            print("  [重叠规避] 跳过 G2 账本持仓 %d 只: %s" % (len(g2_held), ",".join(sorted(g2_held))))
        ...
        for i in range(n_slots):
            ...
            orders.append({
                ...
                "reason": "融合选股Top%d(total=%.1f)" % (i + 1, best["total"]),
...
def main():
    ...
    if not orders:
        ...
        _notify_feishu("【融合换仓 %s】今日无买卖（持仓未满 %d 日，保持 %d 只）" % (date, G.HOLD_DAYS, len(plan.get("ledger", {}))))
        return 0
    for o in orders:
    ...
    if args.live:
        ...
        plan["seq"] = seq
        plan["written"] = True
        _notify_feishu("【融合换仓 %s】已写桥 seq=%d，共 %d 条\n%s" % (
            date, seq, len(orders),
...
def _save_plan(plan, date):
    out_dir = os.path.join(G.DATA_DIR, "rebalance_g2_ens")
    os.makedirs(out_dir, exist_ok=True)
    jp = os.path.join(out_dir, "rebalance_g2_ens_%s.json" % date)
    with open(jp, "w", encoding="utf-8") as f:
        json.dump(plan, f, ensure_ascii=False, indent=2)
    mp = os.path.join(out_dir, "rebalance_g2_ens_%s.md" % date)
