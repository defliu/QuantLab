...
## 八·五、外部方案对照评估（2026-08-31 增补）

> 用户提供《基于大QMT的量化交易执行框架：架构设计与实施方案》（今日头条技术文章）要求参考评估。结论：**文章「信号桥」架构与本案文件桥同源**，方向印证；个别工程点记录为后续增强项，首版不引入。

### 8.5.1 已印证（我们已有，无需改）

| 文章要点 | 本案对应 |
|---|---|
| 策略层不导入 QMT 库，文件驱动执行器 | 信号层全留外部 Python3.10，内置桥零第三方库 |
| 原子写（os.replace 防半 JSON） | `tmp + rename` 原子写（外部桥客户端已实现） |
| 委托后反查 `get_trade_detail_data` 拿 sysid 确认 | pending 状态机短轮询 + sysid 回写 fills |
| 多源降级 | 8 级降级链（TDX→腾讯→iFind→东财→…→miniQMT） |
| 账号隔离 | meta.json account_id 戳 + fail-safe（T-20260823-004） |

### 8.5.2 记录为后续增强项（首版不阻塞主链路）

| 增强项 | 说明 | 优先级 |
|---|---|---|
| **卖出 GATE 多层门禁 + 数据新鲜度校验** | 核心止损前校验：开盘初期不执行 / 情绪过滤 / Tick 动量 / 反弹暂缓；过期 Tick 不用于决策 | P1（V2.0 主链路跑通后叠加） |
| **撤单冷却期（3 秒）** | 防反复撤单；本案 5 分钟超时重报频率低，顺手可加 | P2 |
| **动态风控参数** | 情绪阶段调节仓位上限/池容量；本案已有「大盘 T 值」(0/1/2) 方向一致，粒度可细化 | P2 |

### 8.5.3 明确不引入（过度设计）

- **Redis 双中心**：针对多进程/多机高频信号流；本案单机低频（每日 1-2 次换仓），文件桥够用。
- **0.5s 高频循环 + Tick 预取线程**：面向盘中秒级买入引擎；P16 为 09:25/09:45 固定时点评分换仓。
- **PyQt5 GUI / 进程守护 / 远程同步**：Windows 计划任务已覆盖守护。

### 8.5.4 首版范围拍板（2026-08-31 用户确认）

1. 首版落地**主链路**：读指令 → passorder → 反查 → fills/heart；GATE 作后续。
2. 信号文件粒度：**当日合并单文件**（`cmd/orders_<date>.json` + seq 幂等），非文章的单信号单文件。
